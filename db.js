...
const initDB = () => {
    return new Promise(async (resolve, reject) => {
        ...
        // ENTERPRISE FIX: The Database Deadlock Shield!
        // If a background tab is locking the old database, force a safe reload to break the lock!
        request.onblocked = () => {
            console.warn("🔒 IndexedDB is locked by an older version. Forcing safety reload...");
            alert("Database upgrading. The app will quickly restart to apply changes!");
        ...
        if (existingRecord) {
            ...
            if (docNo && partyId) {
                ...
                for (const r of receipts) {
                    ...
                    // Purchases use 'linkedInvoice' instead of 'invoiceRef'. If we don't check both, editing a Purchase leaves a duplicate ghost payment!
                    if ((String(r.invoiceRef) === String(docNo) || String(r.linkedInvoice) === String(docNo)) && String(r.ledgerId) === String(partyId) && r.isAutoGenerated) {
