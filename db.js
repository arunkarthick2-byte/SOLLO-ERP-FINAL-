        ...
        if (existingRecord) {
            ...
            if (docNo && partyId) {
                ...
                for (const r of receipts) {
                    // ENTERPRISE FIX: Coerce both to String to prevent orphan receipts from multiplying customer debt during edits!
                    if (String(r.invoiceRef) === String(docNo) && String(r.ledgerId) === String(partyId) && r.isAutoGenerated) {
                        deletedReceiptsSnapshot.push(r); // Safely back it up before deleting!
                        await deleteRecordById('receipts', r.id); 
                    }
                    else if (String(r.ledgerId) === String(partyId) && !r.isAutoGenerated && String(docNo) !== String(newDocNo)) {
                        const refs = String(r.invoiceRef || '').split(',').map(x => x.trim());
                        if (refs.includes(String(docNo))) {
                            const newRefs = refs.map(ref => ref === String(docNo) ? String(newDocNo) : ref);
...
const getNextDocumentNumber = async (storeName, docType, targetField = null) => {
    ...
    firmRecords.forEach(record => {
        ...
        if (targetField) {
            ...
            // ENTERPRISE FIX: Stop Vendor Hijacking! 
            // We only look at our internal poNo. Looking at a supplier's invoiceNo will ruin our sequence!
            docNo = String((storeName === 'sales' ? record.invoiceNo : record.poNo) || ''); 
        }
    ...
    return new Promise((resolve, reject) => {
        ...
        validStores.forEach(storeName => {
            ...
            if (storeName === 'counters' || storeName === 'units' || storeName === 'expenseCategories') {
                // ENTERPRISE FIX: Removed store.clear() to prevent destroying Firm A's settings when importing Firm B!
                if (Array.isArray(parsedData[storeName])) {
                    parsedData[storeName].forEach(record => {
                        store.put(record); // Safely upsert without wiping out other firms
                    });
                }
            } else {
...
async function generateGSTReport(yearMonth, firmId) {
    ...
    monthSales.forEach(s => {
        ...
        // ENTERPRISE FIX: Force String comparison so IndexedDB Number IDs match HTML String IDs!
        let customer = ledgers.find(l => String(l.id) === String(s.customerId));
        
        // CRITICAL TAX FIX: A valid GSTIN must be exactly 15 characters! Rejects fake entries like "N/A" or "URP" from corrupting government B2B filings.
        if (customer && customer.gst && customer.gst.trim().length === 15) {
