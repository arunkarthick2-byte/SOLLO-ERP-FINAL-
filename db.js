        ...
        if (oldRecord && (storeName === 'sales' || storeName === 'purchases' || storeName === 'adjustments' || storeName === 'expenses')) {
            ...
            if (uniqueRefs.length > 0) {
                ...
                const receiptsToDelete = receipts.filter(r => 
                    (uniqueRefs.includes(r.invoiceRef) || uniqueRefs.includes(r.linkedInvoice)) && 
                    (!partyId || r.ledgerId === partyId) && 
                    r.isAutoGenerated && 
                    r.firmId === oldRecord.firmId
                );
...
const getNextDocumentNumber = async (storeName, docType, targetField = null) => {
    ...
    firmRecords.forEach(record => {
        ...
        if (targetField) {
            ...
            docNo = String((storeName === 'sales' ? record.invoiceNo : (record.poNo || record.invoiceNo)) || ''); 
        }
...
const getGlobalTimeline = async (firmId) => {
    ...
    // ENTERPRISE FIX: Sort by Date AND ID to prevent same-day transactions from scrambling on the Dashboard!
    return timeline.sort((a, b) => {
        const dateA = new Date(a.date || 0).getTime();
        const dateB = new Date(b.date || 0).getTime();
        if (dateA !== dateB) return dateB - dateA; // Descending Date
        return ((b.id || '') > (a.id || '')) ? 1 : -1; // Descending ID Fallback
    });
};
...
async function generateGSTReport(yearMonth, firmId) {
    // ENTERPRISE FIX: Use native IndexedDB indices to prevent RAM crashes on massive databases!
    const sales = await getAllRecords('sales', 'firmId', firmId);
    const purchases = await getAllRecords('purchases', 'firmId', firmId);
    const ledgers = await getAllRecords('ledgers', 'firmId', firmId);
