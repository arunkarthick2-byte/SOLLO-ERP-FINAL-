    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (oldRecord && storeName !== 'trash') {
                oldRecord._module = storeName; // Track where it came from
                oldRecord._deletedAt = new Date().toISOString(); // Timestamp the deletion
                await saveRecord('trash', oldRecord); // Save to the new trash store
            }
